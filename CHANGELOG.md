...
- `prepare` script (runs `tsup`) so a consumer installing this package directly
  from a git URL — e.g. `maker-toolkit`'s `apps/desktop`, ahead of an npm
  release — gets a built `dist/` automatically. No effect on a normal registry
  install, which already ships a pre-built tarball.
